...
object Application {
  ...
  private def saveTransformed(spark: SparkSession, argsConfiguration: Configuration): Unit = {
    ...
    val processed = spark.sql("SELECT VIN AS `vin`, first(make) AS `make`, first(`year`) AS `year`, max(valueIfInLastXDays(speed, hrTimestamp, 1)) as `maxSpeedToday`, max(valueIfInLastXDays(speed, hrTimestamp, 7)) as `maxSpeedLast7Days`, avg(cast(`speed` AS Double)) AS `avgSpeed`, max(cast(`instantFuelEconomy` AS Double)) AS `bestFuelEconomy`, avg(cast(`instantFuelEconomy` AS Double)) AS `totalFuelEconomy`, cast(count(vin) as Int) as `dataPointCount`, sum(speed) as `speedSum` FROM raw_data GROUP BY vin")
